...
And finally, we need to create an *interpreter*, which is a natural transformation from the free monad to a monad type that handles the evaluation, like a scala `Future` or a scalaz or cats `Task`. Please consult the links above for more elaborate explanation. In the vector space analogy, a natural transformation would correspond to a [linear transformation](http://mathworld.wolfram.com/LinearTransformation.html).
...
Returning once again to our vector space analogy, a coproduct is the equivalent of creating combining two sets of basis vectors of the same dimension with linearly independent vectors, and then `Free[ShipOp, A]` is analogous to the vector space generated by this combined basis set.
